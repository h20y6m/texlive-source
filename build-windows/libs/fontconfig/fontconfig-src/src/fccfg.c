...
/* 
  I allow only one font config directory given by
  XE_FONTCONFIG_PATH or FONTCONFIG_PATH.
  Thus env = NULL in the following.
*/
#if !defined(_WIN32)
    env = (FcChar8 *) getenv ("XE_FONTCONFIG_PATH_EXTRA");
    if (!env)
	env = (FcChar8 *) getenv ("FONTCONFIG_PATH_EXTRA");
    if (env)
	env = strdup(env);
    if (env)
    {
    ...
    if (fontconfig_path[0] == '\0') {
      char *p;
/*
XE_FONTCONFIG_PATH precedes --ak
*/
      p = kpse_var_value("XE_FONTCONFIG_PATH");
      if(p == NULL)
        p = kpse_var_value("FONTCONFIG_PATH");
      if(p == NULL) {
        fprintf(stderr, "I cannot determine FONTCONFIG_PATH. ");
        fprintf(stderr, "Define FONTCONFIG_PATH in texmf.cnf.\n");
        exit(1);
      } else {
        strcpy(fontconfig_path, p);
        free(p);
      }
    }
#endif
...
#ifdef _WIN32
static FcBool	_FcConfigHomeEnabled = FcFalse;
#else
		     ...
		     const FcChar8 *url)
{
    ...
    {
#ifdef _WIN32
	url = (FcChar8 *) kpse_var_value ("FONTCONFIG_FILE");
#else
